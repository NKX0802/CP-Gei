      ...
      <div className="min-h-screen bg-primary-50 pt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5">
            {Array.from({ length: 4 }).map((_, i) => <SkeletonFacilityCard key={i} />)}
          </div>
        </div>
      </div>
    )
    ...
    return (
      <div className="min-h-screen bg-primary-50 pt-16 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/login" className="text-primary-600 font-semibold hover:underline">Go to Login</Link>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
        ...
        {favourites.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-28 text-center">
            ...
            <Link
              href="/facilities"
              className="flex items-center gap-2 px-5 py-2.5 bg-primary-600 text-white rounded-xl text-sm font-semibold hover:bg-primary-700 will-change-transform hover:scale-105 active:scale-95 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500"
            >
              Browse Facilities
            </Link>
          </div>
