...
import { FACILITIES } from '@/lib/fakeData'
...
export default function AdminRoomsPage() {
  ...
  function openEdit(f) {
    setForm({ facility_name: f.facility_name, facility_capacity: f.facility_capacity, facility_type: f.facility_type, facility_status: f.facility_status, facility_description: f.facility_description || '', facility_image_url: f.facility_image_url || '' })
  ...
  function handleImageChange(e) {
    ...
    setImagePreview(url)
    setForm(f => ({ ...f, facility_image_url: url }))
  }
  ...
  function handleSave() {
    if (!form.facility_name || !form.facility_capacity) {
      toast.error('Name and capacity are required.')
      return
    }
    if (modal === 'add') {
      const newId = Math.max(...facilities.map(f => f.facility_id)) + 1
      setFacilities(prev => [...prev, { ...form, facility_id: newId, facility_capacity: Number(form.facility_capacity) }])
      toast.success(`"${form.facility_name}" added successfully!`)
    } else {
      setFacilities(prev => prev.map(f => f.facility_id === editId ? { ...f, ...form, facility_capacity: Number(form.facility_capacity) } : f))
  ...
  function handleDelete(id) {
    const name = facilities.find(f => f.facility_id === id)?.facility_name
    setFacilities(prev => prev.filter(f => f.facility_id !== id))
    setDeleteConfirm(null)
    toast(`"${name}" deleted.`, { icon: '🗑️' })
  }
                ...
                <tr key={f.facility_id} className="hover:bg-gray-50 transition-colors group">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        onClick={() => setDeleteConfirm(f.facility_id)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-10 flex flex-col"
            ...
            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100 shrink-0">
              ...
              <button onClick={() => setModal(null)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-all">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="overflow-y-auto flex-1 px-6 pb-4 pt-5">
              <div className="space-y-5">
                ...
                <Field label="Facility name" required>
                  <input type="text" value={form.facility_name} onChange={e => setForm(f => ({ ...f, facility_name: e.target.value }))}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <Field label="Capacity" required>
                    <input type="number" min="1" value={form.facility_capacity} onChange={e => setForm(f => ({ ...f, facility_capacity: e.target.value }))}
                      placeholder="e.g. 10" className="input-base" />
                  </Field>
                </div>

                <Field label="Status">
                  <select value={form.facility_status} onChange={e => setForm(f => ({ ...f, facility_status: e.target.value }))} className="input-base">
                ...
                <Field label="Description">
                  <textarea value={form.facility_description} onChange={e => setForm(f => ({ ...f, facility_description: e.target.value }))}
            ...
            <div className="flex gap-3 px-6 py-4 border-t border-gray-100 shrink-0">
              <button onClick={handleSave}
                ...
                <CheckCircle2 size={15} /> {modal === 'add' ? 'Add Facility' : 'Save Changes'}
              </button>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 z-10 text-center">
            ...
            <p className="text-sm text-gray-500 mb-5">This action cannot be undone. All bookings for this facility will be affected.</p>
